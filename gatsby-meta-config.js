module.exports = {
  ...
  about: {
    introduce: [
      '5년차 프론트엔드 엔지니어로, 초기 스타트업에서 제품의 설계부터 출시, 운영 개선까지 맡아왔습니다.',
      '종합 법률 서비스 회사의 개발팀 인턴으로 커리어를 시작해, 공유 별장 스타트업 ‘스테이빌리티’에서 프론트엔드를 단독으로 전담하며 예약 서비스와 운영 백오피스를 만들었고, 지금은 럭셔리 여행 플랫폼 ‘아모트래블’의 초기 멤버로 고객 홈페이지와 상담 운영 어드민, 신규 서비스를 담당하고 있습니다. 개인 프로젝트인 모바일 청첩장 서비스 ‘아름’은 혼자 기획·개발·운영하며 월간 이용 6만 규모로 키웠습니다.',
      '현장에서 문제를 발견하는 관찰 · 빠른 실행 · 데이터로 확인하는 개선이라는 세 가지 가치를 추구하며, 이를 바탕으로 비즈니스의 성공에 능동적으로 기여하는 엔지니어가 되고자 합니다.',
    ],
    timestamps: [
      ...
      // ========================================================
      {
        ...
        jobDescriptions: [
          ...
          '트래블러리 — 상담·견적·예약 운영 어드민: 상담·견적·예약을 세 곳에서 각각 수기로 만들고 변경마다 맞춰야 했습니다. 고객에게 전달되는 일정표 하나를 단일 기준으로 삼고 견적과 예약이 자동으로 파생되도록 바꿨습니다. 드래그 앤 드롭 멀티선택·이동과 Tiptap 에디터를 구현했습니다. 사내 작업 로그 실측 기준으로 작성 시간이 단일국가 일정은 약 83%, 다국가 일정은 약 80% 줄었고 도입 후 확인된 싱크 실패는 0건입니다. [일정표 샘플 보기](https://www.travelary.ahmotravel.com/share/6a02bf50-32be-44b5-a843-100536c0ee93)',
          '아모트래블 홈페이지 — 탐색형 전면 리뉴얼: 광고 유입 의존도를 낮추기 위해 문의용 랜딩을 여행지·숙소·고객 후기를 탐색하는 구조로 바꿨습니다. 운영자가 직접 발행하는 사내 CMS를 구축해 연결하고 한·영 다국어와 영문 SEO를 붙였습니다. 운영을 멈추지 않고 기능 단위로 이관하며 진행했습니다. 이 변화들의 복합 기여로 검색 노출 페이지가 초기 기준 대비 약 4,100% 늘었습니다(GSC 기준). [홈페이지](https://www.ahmotravel.com/)',
          '문의 퍼널 — AI 샘플 일정과 전환 개선: 문의를 마친 고객이 상담 전까지 아무 정보도 받지 못하던 구간에, 기존 일정 중 가장 유사한 사례를 AI가 찾아 바로 전달하고 일정표를 고객 계정과 연결했습니다. 배포 전후 각 28일을 비교했을 때 문의량은 거의 같았지만 문의 고객의 계정 연결률은 40.7%p 높아졌습니다. 폼을 단일 스텝에서 멀티스텝으로 바꾸는 안은 A/B 테스트로 전환율 16.7% 개선을 확인한 뒤 적용했고, 지금은 최종 개인정보 입력 단계의 이탈 원인을 다음 검증 과제로 두고 있습니다. [문의 플로우](https://www.ahmotravel.com/inquiry?step=1)',
          '한웨이 — 외국인 대상 내한여행 서비스 0에서 1: 디자이너와 2인으로 초기 버전을 5일 만에 출시했습니다(개발 단독). 퍼널을 Compound Component로 패턴화하고 Figma 토큰과 코드의 이름을 맞췄습니다. 이 규칙은 이후 다른 화면과 동료 개발자가 그대로 채택해 팀 공통 규약이 됐습니다. [한웨이](https://www.hanway.travel/)',
          '모아트래블 — 그룹투어 호스트 모집: 호스트 모집 페이지와 지원 플로우, 계약 후 운영 화면을 만들었습니다. 추천 링크 발급에서 문의 귀속, 계약금 완료 보상까지 이어지는 흐름도 구현했습니다. [모아트래블](https://www.ahmotravel.com/moah)',
          '사내 도구와 계측 기반: Slack 자연어 요청을 Jira 티켓과 사내 지표 조회로 잇는 AI 봇을 제안해 만들었습니다. 데이터베이스는 읽기 전용과 조회 컬럼 제한으로 민감 정보 접근을 막았습니다. Google Analytics·PostHog·Search Console·메타픽셀 계측도 직접 구축해 출시 이후의 판단 근거를 마련했습니다.',
        ],
        date: '2024.09 ~ ',
      ...
      },
      {
        ...
        jobDescriptions: [
          ...
          '예약 서비스 개발: 밀리언그라운드에 필요한 예약 서비스 전체를 설계하고 개발해 별장 구매자와 일반 사용자가 간편하게 예약할 수 있도록 했습니다. 예약·결제 오류율은 1% 미만으로 유지했습니다. 신규 브랜드 ‘어나더 빌라’의 예약 서비스를 출시해 첫해 예약 전량 마감에 기여했습니다. [밀리언그라운드](https://www.millionground.com/)',
